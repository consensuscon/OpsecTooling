...
def configure_node_exporter():
    os.chdir("/home/sysadmin/")
    make_node_user = subprocess.Popen('sudo useradd --no-create-home --shell /bin/false node_exporter', shell=True, stdin=None, executable="/bin/bash")
    make_node_user.wait()
    ...
    set_permissions = subprocess.Popen('sudo cp node_exporter-0.18.1.linux-amd64.tar.gz/node_exporter /usr/local/bin && sudo chown node_exporter:node_exporter /usr/local/bin/node_exporter',
                                        shell=True, stdin=None, executable="/bin/bash")
    set_permissions.wait()
    os.chdir("/home/sysadmin/")
    clean_up = subprocess.Popen('sudo rm -rf node_exporter-0.18.1.linux-amd64', shell=True, stdin=None, executable="/bin/bash")
    clean_up.wait()
